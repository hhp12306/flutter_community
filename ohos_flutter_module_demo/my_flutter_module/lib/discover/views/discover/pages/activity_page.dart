...
class _ActivityPageState extends State<ActivityPage>
  ...
  Future<void> _openCitySelector() async {
    try {
      // 使用路由跳转，确保全屏显示
      // 将 CityModel 转换为 Map 传递（GetX arguments 需要可序列化的数据）
      final selectedCity = await Get.toNamed<CityModel>(
        AppRoutes.citySelector,
        arguments: {
          'currentCity': _currentCity?.toJson(), // 转换为 Map
        },
      );
      ...
      // 如果路由跳转失败，使用备用方式：直接跳转 Widget
      debugPrint('路由跳转失败，使用备用方式: $e');
  ...
  /// 打开活动详情页（H5）
  /// 在H5详情页中，点击"参与话题"按钮会触发跳转到发帖页
  void _openActivityDetail(ActivityModel activity) {
    // 构建H5详情页URL（示例URL，实际应该从后端获取）
