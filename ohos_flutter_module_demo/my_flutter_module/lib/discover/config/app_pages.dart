...
import '../views/h5/h5_webview_page.dart';
import '../views/common/city_selector_page.dart';
...
class AppPages {
  static final List<GetPage> routes = [
    ...
    // H5 WebView页面
    GetPage(
      name: AppRoutes.h5WebView,
      page: () {
        final url = Get.parameters['url'] ?? Get.arguments?['url'] ?? '';
        final title = Get.parameters['title'] ?? Get.arguments?['title'];
        final paramsParam = Get.arguments?['params'];
        
        Map<String, dynamic>? params;
        if (paramsParam != null && paramsParam is Map) {
          params = Map<String, dynamic>.from(paramsParam);
        }
    ...
    GetPage(
      ...
      page: () {
        ...
        // 支持从 Map 或 CityModel 对象恢复
        if (currentCityParam != null) {
          if (currentCityParam is CityModel) {
            currentCity = currentCityParam;
          } else if (currentCityParam is Map<String, dynamic>) {
            // 从 Map 创建 CityModel
            currentCity = CityModel.fromJson(currentCityParam);
          }
        }
